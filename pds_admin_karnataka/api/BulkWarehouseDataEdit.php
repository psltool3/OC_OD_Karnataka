	...
	//if (isset($_POST["submit"])){
		...
		if ($_FILES["file"]["size"] > 0) {
			...
			while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
				if($i>0){
					if($district<0 or $taluka<0 or $name<0 or $id<0 or $type<0 or $storage<0 or $latitude<0 or $longitude<0 or $warehousetype<0 or $active<0 or $ragi<0 or $jowar<0){
					...
					for($j=0;$j<count($column);$j++){
						switch($column[$j]){
							...
							case $reverseMapData["warehousetype"]:
								$warehousetype = $j;
								break;
							case $reverseMapData["ragi"]:
								$ragi = $j;
								break;
							case $reverseMapData["jowar"]:
								$jowar = $j;
								break;
				...
				if($i>0){
					...
					for($j=0;$j<count($column);$j++){
						switch($column[$j]){
							...
							case $reverseMapData["ragi"]:
								$ragi = $j;
								break;						case $reverseMapData["jowar"]:
							$jowar = $j;
							break;							case $reverseMapData["active"]:
								$active = $j;
								break;
						}
					}
