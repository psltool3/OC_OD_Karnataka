...
$query = "SELECT approve_district FROM " . $tablename . " WHERE approve_district='yes' AND (new_id_district IS NULL OR new_id_district='')";
$result = mysqli_query($con,$query);
$totalidsagreed = mysqli_num_rows($result);

$query = "SELECT approve_district FROM " . $tablename . " WHERE approve_district='yes' AND (approve_admin IS NULL OR approve_admin='')";
$result = mysqli_query($con,$query);
$totalidspending = mysqli_num_rows($result);

$query = "SELECT status FROM " . $tablename . " WHERE status='implemented'";
$result = mysqli_query($con,$query);
$totalidsimplemented = mysqli_num_rows($result);
...
$resultarray["totalidsagreed"] = $totalidsagreed;
$resultarray["totalidspending"] = $totalidspending;
$resultarray["totalidsimplemented"] = $totalidsimplemented;
echo json_encode($resultarray);
